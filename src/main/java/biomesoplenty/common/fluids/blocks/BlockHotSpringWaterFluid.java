...
public class BlockHotSpringWaterFluid extends BlockFluidClassic {
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		...
		if (rand.nextInt(25) == 0) {
			world.spawnParticle(EnumParticleTypes.CLOUD, pos.getX() + rand.nextFloat(), pos.getY() + 1.0F,
	...
	public boolean checkForMixing(World worldIn, BlockPos pos, IBlockState state) {
		...
		for (EnumFacing enumfacing : EnumFacing.values()) {
			if (enumfacing != EnumFacing.DOWN
					&& (worldIn.getBlockState(pos.offset(enumfacing)).getMaterial().isLiquid() == true)) {
